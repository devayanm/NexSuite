...
router
  .route("/send-immediate")
  .post(
    isAuthenticated,
    upload.array("attachments", 10),
    emailController.SendImmediateEmail
  );
...
router.route("/active").get(isAuthenticated, emailController.GetAllActiveJobs);

// Route for viewing all emails
router.route("/").get(isAuthenticated, emailController.viewAllEmails);
...
// Route for testing email configuration (no auth required for testing)
router.route("/test").post(emailController.testEmail);
