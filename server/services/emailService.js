...
const scheduleEmailJob = (
  ...
  console.log(
    `Scheduling email to: ${to} at ${cronExpression}, Status: ${status}`
  );
...
const sendImmediateEmail = async (
  ...
  try {
    ...
    throw emailError; // Re-throw to be caught by controller
  }
...
const markEmailAsInactive = async (id) => {
  try {
    ...
    if (job) {
      ...
      emailDetails.delete(id);
      console.log(
        `Job for email ID ${id} has been stopped and marked as inactive.`
      );
    }
